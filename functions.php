...
function ywpi_get_action_name_for_nonce( $document ) {
	return 'ywpi_action_' . $document->exists . yit_get_prop( $document->order, 'order_key' );
}
 ...
 *
 * @param string        $arg_name Arg name.
 * @param YITH_Document $document Document.
 */
function ywpi_document_nonce_url( $arg_name, $document ) {
	$action_url   = add_query_arg( $arg_name, yit_get_prop( $document->order, 'id' ), remove_query_arg( ywpi_get_query_args_list() ) );
